  ...
  const connectPrinter = async () => {
    try {
      ...
      if (device.configuration === null) {
        ...
        for (let configValue = 1; configValue <= device.configurations.length; configValue++) {
          try {
            ...
            console.log(`Failed to select configuration ${configValue}:`, configError)
          }
        }
        if (!configSelected) {
      ...
      if (config && config.interfaces) {
        console.log('Available interfaces:', config.interfaces.map((iface:any) => ({
        ...
        for (const iface of config.interfaces) {
          if (!iface.claimed) {
            try {
              ...
              setStatus(`Interface ${iface.interfaceNumber} claimed successfully`)
              interfaceClaimed = true
              console.log(`Successfully claimed interface ${iface.interfaceNumber}`)
              break
            } catch (claimError) {
              console.log(`Failed to claim interface ${iface.interfaceNumber}:`, claimError)
            }
          }
        }
      }
      ...
      if (!interfaceClaimed) {
        ...
        for (const interfaceNum of commonInterfaces) {
          try {
            ...
            interfaceClaimed = true
            console.log(`Successfully claimed interface ${interfaceNum} via fallback`)
            break
          } catch (claimError) {
            console.log(`Fallback: Failed to claim interface ${interfaceNum}:`, claimError)
          }
        }
      }
      ...
      if (!interfaceClaimed) {
        ...
        try {
          ...
          for (const interfaceNum of [0, 1, 2]) {
            try {
              await device.claimInterface(interfaceNum)
              setStatus(`Interface ${interfaceNum} claimed after force release`)
              interfaceClaimed = true
              break
            } catch (claimError) {
              console.log(`Still failed to claim interface ${interfaceNum}:`, claimError)
            }
          }
        } catch (releaseError) {
    ...
    try {
      ...
      for (const endpoint of commonEndpoints) {
        try {
          ...
          console.log(`Failed to print on endpoint ${endpoint}:`, endpointError)
        }
      }
      
      if (!printSuccess) {
        throw new Error('Could not find working endpoint for printing. Check printer compatibility.')
      }
    } catch (error: any) {
    ...
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      {status.includes('Could not claim any interface') && (
        <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
          <h3 className="font-semibold text-red-800 mb-3">🚨 Device Busy - Quick Fixes:</h3>
          <ol className="text-sm text-red-700 space-y-2 list-decimal list-inside">
            <li><strong>Close printer software:</strong> Star utilities, POS apps, printer drivers</li>
            <li><strong>Disconnect USB:</strong> Unplug for 10 seconds, then reconnect</li>
            <li><strong>Try Reset Connection</strong> button above</li>
            <li><strong>Different USB port:</strong> Try another USB port</li>
            <li><strong>Disable in Device Manager:</strong> Temporarily disable printer driver</li>
